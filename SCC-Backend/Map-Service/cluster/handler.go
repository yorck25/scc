...
func AddClustersGrid(ctx *core.WebContext) error {
	gridRepo := grid.NewRepository(ctx)
	...
	selectedGrid, err := gridRepo.GetGridForCity(cityId)
	if err != nil {
		return err
	}
	...
	newCells, err := GenerateRandomFieldCluster(selectedGrid.Width, selectedGrid.Width, cells, cell.Oil)
	if err != nil {
		return err
	}

	for _, newCell := range newCells {
		err := cellRepo.UpdateCell(newCell)
		if err != nil {
			return ctx.InternalError(err.Error())
		}
	}

	return ctx.Success("Add resources to grid")
}
